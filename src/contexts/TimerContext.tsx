...
export const TimerProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [settings, setSettings] = useState<TimerSettings>(() => {
    ...
    // If settings exist in localStorage, merge them with default settings to ensure all properties exist
    if (saved) {
      ...
      if (parsedSettings.announceMode) {
        migratedSettings.useContinuousCountdown = parsedSettings.announceMode === 'continuous';
        migratedSettings.useSpecificAnnouncements = parsedSettings.announceMode === 'specific';
      }
  ...
  useEffect(() => {
    // Always ensure voice is enabled before saving
    const settingsToSave = { ...settings, useVoice: true };
  ...
  const updateSettings = (newSettings: Partial<TimerSettings>) => {
    // Always ensure voice is enabled
    setSettings(prev => ({ ...prev, ...newSettings, useVoice: true }));
  };
